...
const assertManage = async (role: StaffRole) => { if (!(await hasPermission(role, "staff.manage"))) throw new Error(`Forbidden: role "${role}" cannot manage staff accounts.`); };
...
export async function createDepartment(name: string, description: string | null, actorRole: StaffRole) {
  await assertManage(actorRole); const { data, error } = await getServiceRoleClient().from("departments").insert({ name: name.trim(), description }).select().single(); if (error) throw error;
...
export async function createStaffAccount(input: { email:string; password:string; fullName:string; role:StaffRole; roles?:StaffRole[]; departmentId?:string|null; qualification?:string; designation?:string; phone?:string; actorRole:StaffRole }): Promise<StaffProfile> {
  await assertManage(input.actorRole); const supabase=getServiceRoleClient(); const {data:created,error:createError}=await supabase.auth.admin.createUser({email:input.email,password:input.password,email_confirm:true}); if(createError)throw createError; if(!created.user)throw new Error("Failed to create staff auth user.");
...
export async function assignStaffRoles(staffId:string,roles:StaffRole[],actorRole:StaffRole,primaryRole?:StaffRole):Promise<void>{
  await assertManage(actorRole); const clean=Array.from(new Set(roles)); if(!clean.length)throw new Error("At least one role is required."); const primary=primaryRole&&clean.includes(primaryRole)?primaryRole:clean[0]; const supabase=getServiceRoleClient();
...
export async function deactivateStaffAccount(staffId:string,actorRole:StaffRole){await assertManage(actorRole);const {error}=await getServiceRoleClient().from("staff_profiles").update({is_active:false}).eq("id",staffId);if(error)throw error;await logAudit({action:"STAFF_DEACTIVATED",entityType:"staff_profiles",entityId:staffId,actorRole});}
export async function reactivateStaffAccount(staffId:string,actorRole:StaffRole){await assertManage(actorRole);const {error}=await getServiceRoleClient().from("staff_profiles").update({is_active:true}).eq("id",staffId);if(error)throw error;await logAudit({action:"STAFF_REACTIVATED",entityType:"staff_profiles",entityId:staffId,actorRole});}
export async function countActiveStaff(actorRole:StaffRole){if(!(await hasPermission(actorRole,"staff.manage"))&&actorRole!=="admin")throw new Error(`Forbidden: role "${actorRole}" cannot view the staff directory.`);const {count,error}=await getServiceRoleClient().from("staff_profiles").select("id",{count:"exact",head:true}).eq("is_active",true);if(error)throw error;return count??0;}
export async function listStaffProfiles(actorRole:StaffRole):Promise<StaffDirectoryMember[]>{
  if(!(await hasPermission(actorRole,"staff.manage"))&&actorRole!=="admin")throw new Error(`Forbidden: role "${actorRole}" cannot view the staff directory.`); const supabase=getServiceRoleClient(); const {data,error}=await supabase.from("staff_profiles").select("*, departments(name)").order("full_name"); if(error)throw error; const rows=data??[]; const ids=rows.map(r=>r.id); const {data:assignments,error:roleError}=ids.length?await supabase.from("staff_role_assignments").select("staff_id, role, is_primary").in("staff_id",ids):{data:[],error:null}; if(roleError)throw roleError;
