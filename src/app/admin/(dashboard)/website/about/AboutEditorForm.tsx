...
export function AboutEditorForm({ content }: { content: AboutContent }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <Section number={2} title="Our approach">
        <Field label="Eyebrow" value={form.approachEyebrow ?? ""} onChange={(v) => set("approachEyebrow", v)} />
        <Field label="Heading" value={form.approachHeading ?? ""} onChange={(v) => set("approachHeading", v)} textarea rows={2} />
        <Field label="Who we are" value={form.whoWeAre ?? ""} onChange={(v) => set("whoWeAre", v)} textarea rows={7} />
      </Section>

      <Section number={3} title="About images">
        <Field label="Primary image path / URL" value={form.aboutImagePath ?? ""} onChange={(v) => set("aboutImagePath", v)} />
        <Field label="Primary image alt text" value={form.aboutImageAlt ?? ""} onChange={(v) => set("aboutImageAlt", v)} />
        <Field label="Secondary image path / URL" value={form.aboutSecondaryImagePath ?? ""} onChange={(v) => set("aboutSecondaryImagePath", v)} />
        <Field label="Secondary image alt text" value={form.aboutSecondaryImageAlt ?? ""} onChange={(v) => set("aboutSecondaryImageAlt", v)} />
      </Section>

      <Section number={4} title="Mission, vision, values">
      ...
      <Section number={5} title="Quality & standards" description="Only include certification/accreditation text once it has actually been confirmed and supplied.">
        <Field label="Quality heading" value={form.qualityHeading ?? ""} onChange={(v) => set("qualityHeading", v)} />
        <Field label="Quality statement" value={form.qualityStatement ?? ""} onChange={(v) => set("qualityStatement", v)} textarea />
        <Field label="Professional standards heading" value={form.professionalStandardsHeading ?? ""} onChange={(v) => set("professionalStandardsHeading", v)} />
