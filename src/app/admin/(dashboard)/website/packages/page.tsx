...
export const metadata: Metadata={title:"packages | Salem Staff Area",robots:{index:false,follow:false}};
export default async function Page(){const staff=await requireStaff();const navItems=getAdminNavItems(staff);if(!can(staff,"settings.manage"))return <AdminShell eyebrow="Website" title="Not available for your role" staffName={staff.fullName} staffRole={staff.role} navItems={navItems}><p className="surface-card p-6 text-sm text-muted-foreground">Your role does not have access to website content.</p></AdminShell>;const page=await getWebsitePage("packages",staff.role);return <AdminShell eyebrow="Website · Staff Area" title="packages" lead="Edit the draft, preview it, then publish when the content is ready." backTo="/admin/website" backLabel="Back to website" staffName={staff.fullName} staffRole={staff.role} navItems={navItems}><div className="space-y-6"><WebsiteContentPublishBar pageKey="packages" status={page.status} updatedAt={page.updated_at} publishedAt={page.published_at}/><div className="mt-6"><PackagesEditorForm content={(page.draft_content as unknown as PackagesContent) ?? {}} /></div></div></AdminShell>}