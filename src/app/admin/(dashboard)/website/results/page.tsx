...
export const metadata: Metadata={title:"results | Salem Staff Area",robots:{index:false,follow:false}};
export default async function Page(){const staff=await requireStaff();const navItems=getAdminNavItems(staff);if(!can(staff,"settings.manage"))return <AdminShell eyebrow="Website" title="Not available for your role" staffName={staff.fullName} staffRole={staff.role} navItems={navItems}><p className="surface-card p-6 text-sm text-muted-foreground">Your role does not have access to website content.</p></AdminShell>;const page=await getWebsitePage("results",staff.role);return <AdminShell eyebrow="Website · Staff Area" title="results" lead="Edit the draft, preview it, then publish when the content is ready." backTo="/admin/website" backLabel="Back to website" staffName={staff.fullName} staffRole={staff.role} navItems={navItems}><div className="space-y-6"><WebsiteContentPublishBar pageKey="results" status={page.status} updatedAt={page.updated_at} publishedAt={page.published_at}/><div className="mt-6"><SimpleContentEditor pageKey="results" type="results" content={(page.draft_content as ResultsContent) ?? {}} /></div></div></AdminShell>}