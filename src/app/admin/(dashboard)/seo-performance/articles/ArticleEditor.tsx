...
export function ArticleEditor({ articles }: { articles: Article[] }) {
  ...
  const handleSave = () => {
    ...
    setError("");
    startSaving(async () => {
      const result = await saveArticleAction(new FormData(form));
      if (result.ok && result.article) {
        ...
        setError(result.error || "Could not save the article.");
      }
    });
  };
  ...
  const handlePublish = () => {
    ...
    startPublishing(async () => {
      // Save the current editor contents first so Publish never publishes stale data.
      const draftData = new FormData(form);
      draftData.set("status", "draft");
      const saved = await saveArticleAction(draftData);
      if (!saved.ok || !saved.article) {
        setError(saved.error || "Save failed. The article was not published.");
        return;
      }
      ...
      publishData.set("id", article.id || "");
      const result = await publishArticleAction(publishData);
      if (result.ok && result.article) {
  ...
  const uploadImage = async (formData: FormData) => {
    ...
    try {
      ...
      if (result.ok && result.imageUrl) {
        ...
        const hiddenImage = formRef.current?.elements.namedItem("featured_image_url") as HTMLInputElement | null;
        if (hiddenImage) hiddenImage.value = result.imageUrl;
  ...
  return (
    <div className="space-y-6">
      ...
      {selected ? (
        <form ref={formRef} key={selected.id || "new-article"} onSubmit={(event) => { event.preventDefault(); handleSave(); }} className="surface-card overflow-hidden">
          ...
          <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_320px]">
            <div className="space-y-5">
              <div><label className="text-sm font-semibold text-navy-deep">Article title</label><input name="title" defaultValue={selected.title} required onBlur={(event) => { if (!selected.slug) { const slug = formRef.current?.elements.namedItem("slug") as HTMLInputElement | null; if (slug) slug.value = slugify(event.currentTarget.value); } }} className="mt-2 w-full rounded-xl border border-input bg-background p-3.5 text-sm outline-none ring-offset-background focus:ring-2 focus:ring-ring" placeholder="e.g. How to Prepare for a Blood Test" /></div>
            ...
            <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-2xl border border-border bg-secondary/50 p-5">
                ...
                <div className="mt-4 aspect-[16/10] overflow-hidden rounded-2xl border border-border bg-background">
