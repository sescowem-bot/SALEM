...
export default async function FaqPage(){const content=await getPublishedPageContent<FaqContent>("faq");const c={...defaultFaqContent,...content}; const groups=content.groups?.length ? content.groups : defaultFaqContent.groups; const title=c.pageTitle ?? defaultFaqContent.pageTitle; const introduction=c.introduction ?? defaultFaqContent.introduction; const faqSchema={"@context":"https://schema.org","@type":"FAQPage",mainEntity:groups.flatMap(g=>g.items).map(item=>({"@type":"Question",name:item.question,acceptedAnswer:{"@type":"Answer",text:item.answer}})),url:canonical("/faq")};return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(faqSchema)}}/><SiteLayout><FaqPageContent content={content}/></SiteLayout></>;}
