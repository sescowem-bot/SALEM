...
import { Droplet, Microscope, Dna, HeartPulse, FlaskConical, Baby } from "lucide-react";

function CategoryIcon({ category }: { category: string }) {
  const value = category.toLowerCase();
  if (value.includes("haemat") || value.includes("blood")) return Droplet;
  if (value.includes("micro")) return Microscope;
  if (value.includes("horm") || value.includes("endocr")) return HeartPulse;
...
export function PremiumNoImage({
  name,
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-gradient-to-br from-navy-deep via-navy to-purple/80">
      ...
      <div className="relative flex h-full flex-col justify-between p-6 text-white">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-soft/80">{category}</p>
          <p className="mt-2 max-w-[15rem] text-xl font-semibold leading-tight">{name}</p>
        </div>
      </div>
    </div>
  );
